package com.example.youpart7sep.main
    ...
    fun getAllTasks(): MutableList<Task> {
        ...
        if (cursor != null && cursor.count > 0) {
            ...
            do {
                ...
                val title = cursor.getInt(cursor.getColumnIndex(TITLE))
                val desc = cursor.getInt(cursor.getColumnIndex(DESC))
//
//                val task = Task()
//                task.ID = id
//                task.income = title
...
//                taskList.add(task)
    ...
    fun editTask(task: Task) {
        ...
        contentValues.put(TITLE, task.income)
        contentValues.put(DESC, task.Exp)
